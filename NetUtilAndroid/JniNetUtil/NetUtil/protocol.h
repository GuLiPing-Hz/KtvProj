...
enum LOBBY_CMD_XGCLIENT
{
	...
	CMD_KTV_GET_XG_USERNUM_C2L2C,    //ktv客户端请求查看某房间的人数和是否有主持
	CMD_KTV_REQUEST_USER_C2L,      //ktv请求主播互动通知
	CMD_GETUSERLISTANDLOGINWAY_L2C,		//服务器发送手机用户列表以及在线方式
	CMD_USERLOGIN_NEW_L2C,  //通知给其他用户大厅用户登陆,加type
	CMD_SEND_REDGOLD_C2L2C,    //发红包
	CMD_RECV_REDGOLD_L2C,  //收红包
	CMD_VIEFOR_REDGOLD_C2L2C,  //抢红包
	CMD_WATCH_REDGOLD_C2L2C,    //查看红包领取记录

	CMD_KTV_SWITCH_USER_STATE_C2L2C, //切换用户状态 空闲/互动中
	CMD_KTV_REQUEST_FEEDBACK_C2L,    //主播收到互动请求后的反馈
	CMD_KTV_FEEDBACK_INFO_L2C,   //包厢邀请者收到主播反馈信息
	CMD_UPDATE_LASTSONG_INFO_L2C,  //更新最近演唱歌曲信息
	CMD_UPDATE_REDNUM_L2C,       //更新红包数量
	CMD_CAN_GET_RED_C2L2C,      //查看红包是否已抢完 0-可以抢 1-已抢完 2-已抢过
	CMD_GET_ROOM_REDPACKET_C2L2C,  //获取当前包厢红包信息
...
enum HALL_ROOM_CMD
{
	...
	HALL_CMD_GAMEBROAD_R2L,     //深海捕鱼喇叭
	HALL_CMD_MOBILEONLINE_R2L,  //上传房间在线用户列表 
	HALL_CMD_PUBLICCHAT_L2R,      //手机和星光用户公聊
	HALL_CMD_MOBILE_SENDGIFT_L2R,  //手机送礼
	HALL_CMD_LUCKY_L2R,   //幸运礼物中奖通知
	HALL_CMD_XG_SENDGIFT_R2L,  //星光向手机用户送礼
	HALL_CMD_KTV_GET_USERNUM_L2R2L,     //ktv客户端请求查看某房间的人数和是否有主持
	HALL_CMD_PUBLICCHAT_R2L,     //星光和手机用户公聊
	HALL_CMD_LUCKY_R2L,    //星光用户幸运礼物中奖通知
	HALL_CMD_GETROOMUSER_L2R2L,  //获取当前房间用户列表
	HALL_CMD_UPDATE_RECVFLOWERCOUNT_R2L,	//更新花花接收数
	HALL_CMD_GETKTVUSER_R2L2R,     //获取ktv主播的状态
	HALL_CMD_REQUESTUSER_R2L2R,    //请求主播互动
	HALL_CMD_LOGOUTROOM_L2R2L,     //ktv客户端注销用户
};
...
enum ROOM_SERVER_CMD  //cmd中有SB为广播消息
{
	...
	ROOM_CMD_SEABED_C2S2C,    //深海捕鱼
	ROOM_CMD_CHECKLOSS,       //检验亏损
	ROOM_CMD_KJ,          //开奖
	ROOM_CMD_SETTLE,      //游戏结算
	//add by caoxinping 2015/1/9 begin
	ROOM_CMD_UP_ONMICLOSSRATE,     //上麦报告上行丢包率  
	ROOM_CMD_SB_ONMICLOSSRATE,     //房间广播丢包率
	ROOM_CMD_SET_DICENUM_C2S,   //设置色子个数
	ROOM_CMD_DICE_START_C2S2C,        //开始摇色子
	ROOM_CMD_SB_DICE_END,        //开广播
	ROOM_CMD_SB_DICETIMES,       //摇色子次数
	ROOM_CMD_DICE_OPEN_C2S,      //开
	//end
	//add by caoxinping 2015/4/18 
	ROOM_CMD_SEND_FREEFLOWER_C2S,   //赠送免费花花 
	ROOM_CMD_UPDATE_RECVFLOWERCOUNT_S2C,	//更新用户接收免费花花数
	ROOM_CMD_GETUSERINFO_K2S2K,   //ktv获取用户信息
	ROOM_CMD_REQUESTUSER_K2S2K,   //ktv请求主播互动
	ROOM_CMD_KTV_REQUEST_FINISH_K2S,    //包厢用户请求结束互动
	ROOM_CMD_KTV_FINISH_NOTIFY_S2K,     //主播收到结束互动通知

	ROOM_CMD_SPLITER_1,  //分隔cpp文件命令处理

	ROOM_CMD_SB_SEABET = 10021,    //游戏广播
	//无意义
	ROOM_CMD_MAX
};
...
enum USER_IDENTITY//room
{
	...
	USER_ID_KING = 17,  //至尊王者
	USER_ID_EMPEROR = 18,   //至尊帝王
	USER_ID_PRINCESS = 19,   //公主冠
};
...
enum VIP_LEVEL				//vip等级
{
	...
	VIP_LEVEL_KING = 500,  //至尊王者
	VIP_LEVEL_EMPEROR = 600,   //至尊帝王
	VIP_LEVEL_PRINCESS = 700,   //公主冠
};
...
enum DBRESULT_GIFT	//礼物、道具赠送
{
	...
	DBRESULT_GIFT_NUM       =-2,     //花花个数不足
	DBRESULT_GIFT_FAILED    =-3,    //送花花失败
};
