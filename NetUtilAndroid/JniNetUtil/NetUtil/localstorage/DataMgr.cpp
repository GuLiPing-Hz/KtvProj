    ...
    switch (e)
    {
        case URL_GIFT:
        {
            if(downloadInfo->info.saveBuf)
            {
                ...
                TiXmlDocument doc;
				doc.Parse((const char*)downloadInfo->info.saveBuf);
                if(!doc.Error())
                {
                    const char * p = NULL;
...
bool CDataMgr::initRootDir(const char* path)
{
    ...
    else
    {
        ...
        CFileMgr::CreateDir(m_dataConfig);
        CFileMgr::CreateDir(m_dataImages);
        CFileMgr::CreateDir(m_dataUpdate);
    }
