...
/*
	...
	@param gold : 红包总金额
	@param mark : 红包备注
	@param extradata : 如果是个人红包，则代表接受者的IDX；如果是房间红包，则代表是房间ID；如果是大厅，则是0
	*/
int RequestBase::RequestSendHB(char hbtype,int number,int gold,const char* mark,int extradata)
{
	if(hbtype < 0 || hbtype > 4 || number == 0 || gold == 0)
		return -1;

	if(hbtype == 0 && extradata == 0)
		return -1;

	if((hbtype == 1 || hbtype == 2) && extradata == 0)
		return -1;

	SendHB* pData = (SendHB*)malloc(sizeof(SendHB));
	if(!pData)
		return -1;
	...
	if(mark)
		strcpy(pData->mark,mark);
	else
		pData->mark[0] = 0;

	pData->extradata = extradata;

	if(g_NetApp.PostMessage(KTV_MOBILE_SENDHB_C2L2C, pData) != 0)
	{
		free(pData);
		return -1;
	}
...
int RequestBase::RequestHBStatus(const char* hbid)
{
	if(!hbid)
		return -1;

	char* pData = (char*)malloc(HONGBAO_ID_LEN);
	if(!pData)
		return -1;

	strcpy(pData,hbid);

	if(g_NetApp.PostMessage(KTV_MOBILE_LOOKHB_C2L2C, pData) != 0)
	{
		free(pData);
		return -1;
	}
...
@param hbid : 红包ID
*/
int RequestBase::RequestVieHB(const char* hbid)
{
	if(!hbid)
		return -1;
	...
	if(g_NetApp.PostMessage(KTV_MOBILE_VIEHB_C2L2C, pData) != 0)
	{
		free(pData);
		return -1;
	}
	...
	查看红包领取详细情况 - 只针对手气红包
	*/
int RequestBase::RequestHBDetail(const char* hbid)
{
	if(!hbid)
		return -1;
	...
	if(g_NetApp.PostMessage(KTV_MOBILE_HBDETAIL_C2L2C, pData) != 0)
	{
		free(pData);
		return -1;
	}
...
int RequestBase::RequestCurrentRoomHB(int roomid)
{
	if(roomid == 0)
		return -1;

	int* pData = (int*)malloc(sizeof(roomid));
	if(!pData)
		return -1;

	if(g_NetApp.PostMessage(KTV_MOBILE_ROOMHB_C2L2C,pData)!=0)
	{
		free(pData);
		return -1;
	}
