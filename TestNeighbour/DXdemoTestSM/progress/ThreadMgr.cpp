...
bool CThreadMgr::Create( int iPriority/* = THREAD_PRIORITY_NORMAL */ )
{
	DWORD dwThreadID;
	...
	if (THREAD_PRIORITY_NORMAL != iPriority)
	{
		...
		{
			//writeLog("CThreadMgr::Create SetThreadPriority error");
		}
	}
...
void CThreadMgr::PostMessage( int id, void * lParam, void * rParam )
{
	_tMsg msg = { false, id, lParam, rParam };
...
bool CThreadMgr::SendMessage( int id, void * lParam, void * rParam )
{
	_tMsg msg = { true, id, lParam, rParam };
...
DWORD CThreadMgr::Run()
{
	while ( !m_bStop )
	{
		...
		{
			ProcessMessage( msg.id, msg.wParam, msg.lParam );
			if ( msg.isSend )
