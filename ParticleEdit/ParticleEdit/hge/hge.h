...
public:
	...
	virtual HCHANNEL CALL   Effect_Pause(HEFFECT eff)=0;
	virtual HCHANNEL CALL	Effect_Stop(HEFFECT eff)=0;
	virtual HCHANNEL CALL   Effect_Resume(HEFFECT eff)=0;
	...
	virtual HTEXTURE	CALL	Texture_Load(const char *filename,unsigned int& w,unsigned int& h, DWORD size=0, bool bMipmap=false) = 0;
