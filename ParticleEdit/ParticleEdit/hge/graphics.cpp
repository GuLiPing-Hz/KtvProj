...
void CALL HGE_Impl::Gfx_RenderLine(float x1, float y1, float x2, float y2, DWORD color, float z)
{
	if(VertArray)
	{
		if(CurPrimType!=HGEPRIM_LINES || nPrim>=VERTEX_BUFFER_SIZE/HGEPRIM_LINES || CurTexture || CurBlendMode!=BLEND_DEFAULT)
		{
			...
			if(CurBlendMode != BLEND_DEFAULT) setBlendMode(BLEND_DEFAULT);
...
void CALL HGE_Impl::Gfx_RenderTriple(const hgeTriple *triple)
{
	if(VertArray)
	{
		if(CurPrimType!=HGEPRIM_TRIPLES || nPrim>=VERTEX_BUFFER_SIZE/HGEPRIM_TRIPLES || CurTexture!=triple->tex || CurBlendMode!=triple->blend)
		{
			...
			if(CurBlendMode != triple->blend) setBlendMode(triple->blend);
...
hgeVertex* CALL HGE_Impl::Gfx_StartBatch(int prim_type, HTEXTURE tex, int blend, int *max_prim)
{
	if(VertArray)
	{
		...
		if(CurBlendMode != blend) setBlendMode(blend);
		if(tex != CurTexture)
		{
...
HTARGET CALL HGE_Impl::Target_Create(int width, int height, bool zbuffer)
{
	...
	if(FAILED(D3DXCreateTexture(pD3DDevice, width, height, 1, D3DUSAGE_RENDERTARGET,
						d3dpp->BackBufferFormat, D3DPOOL_MANAGED/*D3DPOOL_DEFAULT*/, &pTarget->pTex)))
	{
...
HTEXTURE CALL HGE_Impl::Texture_Load(const char *filename, unsigned int& w,unsigned int& h,DWORD size, bool bMipmap)
{
	void *data;
	DWORD _size;
	D3DFORMAT fmt1, fmt2;
...
//	if( FAILED( D3DXCreateTextureFromFileInMemory( pD3DDevice, data, _size, &pTex ) ) ) pTex=NULL;
	...
	w = texItem->width=info.Width;
	h = texItem->height=info.Height;
...
void HGE_Impl::setBlendMode(int blend)
{
	int nColorBlend = blend & BLEND_COLORMASK;
	if(nColorBlend != (CurBlendMode & BLEND_COLORMASK))
	{
		switch(nColorBlend)
		{
		case BLEND_COLORMUL:
	...
	if(nAlphaBlend != (CurBlendMode & BLEND_ALPHAMASK))
	{
		switch(nAlphaBlend)
		{
		case BLEND_ALPHAADD:
	...
	int nZBlend = blend & BLEND_ZMASK;
	if(nZBlend != (CurBlendMode & BLEND_ZMASK))
	{
		switch(nAlphaBlend)
		{
		case BLEND_NOZWRITE:
	...
	while(target)
	{
		if(target->pTex)
			D3DXCreateTexture(pD3DDevice, target->width, target->height, 1, D3DUSAGE_RENDERTARGET,
							  d3dpp->BackBufferFormat, D3DPOOL_MANAGED/*D3DPOOL_DEFAULT*/, &target->pTex);
		if(target->pDepth)
...
// 	pD3DDevice->SetRenderState( D3DRS_ALPHAFUNC, D3DCMP_GREATEREQUAL );//alpha测试函数
	...
	pD3DDevice->SetSamplerState(0,D3DSAMP_MIPFILTER,D3DTEXF_LINEAR/*D3DTEXF_POINT*/);//速度最快，效果比较差
