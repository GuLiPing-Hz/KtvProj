...
public:
	...
	virtual bool		CALL	Resource_AttachPack(const char *filename, const char *password=0){return false;}
	...
	virtual HCHANNEL CALL   Effect_Pause(HEFFECT eff);
	virtual HCHANNEL CALL	Effect_Stop(HEFFECT eff);
	virtual HCHANNEL CALL   Effect_Resume(HEFFECT eff);
	...
	virtual HTEXTURE	CALL	Texture_Load(const char *filename, unsigned int& w,unsigned int& h,DWORD size=0, bool bMipmap=false);
