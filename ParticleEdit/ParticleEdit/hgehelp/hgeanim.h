...
** HGE Animation class
*/
class hgeAnimation : public CGLSprite
{
public:
	...
	void		SetTexture(HTEXTURE tex) { CGLSprite::SetTexture(tex); orig_width = hge->Texture_GetWidth(tex, true); }
	void		SetTextureRect(float x1, float y1, float x2, float y2) { CGLSprite::SetTextureRect(x1,y1,x2,y2); SetFrame(nCurFrame); }
