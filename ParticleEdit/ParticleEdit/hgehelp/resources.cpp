...
{
	unsigned int w,h;
	if(!handle) handle=(DWORD)hge->Texture_Load(filename,w,h, 0, mipmap);
	return handle;
}
...
DWORD RSprite::Get(hgeResourceManager *rm)
{
	...
	if(!handle)
	{
		spr = new CGLSprite(rm->GetTexture(texname, resgroup), tx, ty, w, h);
		spr->SetColor(color);
		spr->SetZ(z);
...
void RSprite::Free()
{
	if(handle) delete (CGLSprite *)handle;
	handle=0;
}
...
DWORD RParticle::Get(hgeResourceManager *rm)
{
	...
	if(!handle)
	{
		par = NULL;//new hgeParticleSystem(filename, rm->GetSprite(spritename));
