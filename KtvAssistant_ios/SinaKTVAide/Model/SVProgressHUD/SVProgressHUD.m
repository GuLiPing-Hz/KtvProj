...
+ (void)showWithTips:(NSString *)str {
    [SVProgressHUD show];
    [[SVProgressHUD sharedView] dismissWithStatus:str afterDelay:1];
}
...
- (void)setStatus:(NSString *)string {
    ...
    CGFloat y = 0;
    if (!spinnerView.isAnimating && imageView.hidden) {
        y += 58;
    }
    if(string) {
        ...
        if(hudHeight > 100) {
            labelRect = CGRectMake(12, 66 - y, hudWidth, stringHeight);
            hudWidth+=24;
        } else {
            hudWidth+=24;  
            labelRect = CGRectMake(0, 66 - y, hudWidth, stringHeight);
        }
    }
	
	self.hudView.bounds = CGRectMake(0, 0, hudWidth, hudHeight - y);
