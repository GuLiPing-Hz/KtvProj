...
- (CGSize)sizeThatFits:(CGSize)size {
  ...
  NSDictionary* attribs = @{NSFontAttributeName:self.font};
  CGSize stringSize = [self.text sizeWithAttributes:attribs];
  CGSize zeroSize = [@"0" sizeWithAttributes:attribs];
  CGFloat padding = 0;
  if (sUsesSolidTint) {
    padding = 2;
  }
...
- (void)drawRect:(CGRect)rect {
  ...
  // Draw text
  [self.textColor set];
  NSDictionary* attribsFont = @{NSFontAttributeName:self.font,NSForegroundColorAttributeName:self.textColor};
  [self.text drawAtPoint:CGPointMake(floorf((rect.size.width - textSize.width) / 2.f) + 0.2f,
                                     floorf((rect.size.height - textSize.height) / 2.f) - 1.5f)
                withAttributes:attribsFont];
}
