...
+ (CGSize)adjustSizeWidthWithAttributedString:(NSAttributedString *)attributedString maxHeight:(CGFloat)height {
    CTFramesetterRef framesetter =
    CTFramesetterCreateWithAttributedString((__bridge CFMutableAttributedStringRef)attributedString);
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.attributedString) {
        ...
        CTFrameRef textFrame1 = CTFramesetterCreateFrame(framesetter, CFRangeMake(0, 0), path, NULL);
        ...
        // 获取CTFrame中的CTLine
        CFArrayRef lines = CTFrameGetLines(textFrame1);
        CGPoint origins[CFArrayGetCount(lines)];
        CTFrameGetLineOrigins(textFrame1, CFRangeMake(0, 0), origins);
        ...
        for (CFIndex i = 0; i < CFArrayGetCount(lines); ++i) {
            ...
            for (CFIndex j = 0; j < CFArrayGetCount(runs); ++j) {
                ...
                const CFIndex *index = CTRunGetStringIndicesPtr(run);
                CGFloat xOffset;
                if (index)
                    xOffset = CTLineGetOffsetForStringIndex(line, index[0], NULL);
                else
                    xOffset = 0;
