	...
	case ROOM_CMD_TOKEN:
		{
			...
			if( ! stream.Read( ret ) )
			{
				//////TRACE_INFO((Error, "read re error");
				return -1;
			}
			...
			if( 0 == ret )
			{
				...
				{
					//////TRACE_INFO((Error, "read sessionkey error");
					return -1;
				}
...
// 		}
	...
	case ROOM_CMD_SB_UPDATE_TOPSONG:
		{
			...
			{
				////TRACE_INFO((Error, "read songcount error");
				return -1;
			}

			if (!stream.Read(topmusicscore))
			{
				////TRACE_INFO((Error, "read topmusicscore error");
				return -1;
			}
			...
			rb->RecvTopSongInfo(songcount, topmusicscore, topmusicname, namelenth);
		}
		break;
    //@int listcount 歌曲数量
    //@while
    ...
    case ROOM_CMD_SB_KTV_CUR_SONGLIST:
    {
        ...
        ResultData_SongList* pSonglist = NULL;
        if(listcount > 0)
        {
        	pSonglist = new ResultData_SongList[listcount];

			for (int i=0; i<listcount; i++) {
				unsigned int len;
				PROTOCOL_READ(pSonglist[i].songid);
				PROTOCOL_READSTR(pSonglist[i].songname,len);
				PROTOCOL_READSTR(pSonglist[i].singername,len);
			}
        }
        //安全释放
        SafePointer sp(pSonglist);
        ResponseBase* pRb = g_NetApp.GetResponseHandler();
        if(pRb)
            pRb->RecvKtvSongList(pSonglist, listcount);
        break;
    }
