...
namespace NetworkUtil
{
	...
	{
		public:
			static int XTEAEncrypt(const char* pbyInBuffer, int nInBufferLength, char* pbyOutBuffer, int nOutBufferLength, char arrbyKey[16]);
			static int XTEADecrypt(const char* pbyInBuffer, int nInBufferLength, char* pbyOutBuffer, int nOutBufferLength, char arrbyKey[16]);
			 ...
			 */
			static bool TEAEncrypt(const char* pInBuffer, int nInBufferLen, char* pOutBuffer, int* pnOutBufferLen, char pKey[16], unsigned int uRounds = 16);
			 ...
			 */
			static bool TEADecrypt(const char* pInBuffer, int nInBufferLen, char* pOutBuffer, int* pnOutBufferLen, char pKey[16], unsigned int uRounds = 16);
	};
