...
void ReserveDataV::OnTimeOut()
{
	...
	LOGI("%s : serverid = %d,cmd = %d\n",__FUNCTION__,serverid,cmd);
	...
	else if (serverid == LOBBYTUNNEL)
	{
        ...
        case CMD_GETPARTINFO_C2L2C:
            {
                if(pRb)
                    pRb->RecvPartInfoBySelf( -1, OPTION_TIMEOUT, NULL, cseq);
                break;
            }
		case CMD_GET_ROOM_REDPACKET_C2L2C:
			{
				if(pRb)
					pRb->RecvRoomHB(OPTION_TIMEOUT,0,NULL);
				break;
			}
		case CMD_WATCH_REDGOLD_C2L2C:
			{
				if(pRb)
					pRb->RecvHongBaoDetail(OPTION_TIMEOUT,0,0,"",0,0,0,NULL,0);
				break;
			}
		case CMD_CAN_GET_RED_C2L2C:
			{
				if(pRb)
					pRb->RecvHongBaoStatus("",OPTION_TIMEOUT);
				break;
			}
		case CMD_VIEFOR_REDGOLD_C2L2C:
			{
				if(pRb)
					pRb->RecvVieHongBaoResult(OPTION_TIMEOUT,0,"",0,"",0);
				break;
			}
		case CMD_SEND_REDGOLD_C2L2C:
			{
				if(pRb)
					pRb->RecvSendHBResult(OPTION_TIMEOUT);
				break;
			}
