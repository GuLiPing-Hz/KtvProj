...
public:
	...
	/******************认证服务器操作接口*****************************/
	//接收认证结果
	virtual void RecvAuth(int ret, int errcode, int again) = 0;
	//接收新token
	...
	*/
	 ...
	 *通知用户昵称修改
	 */
	virtual void RecvUserNameChange(int idx,const char* name) = 0;
	/*
	通知获取到的喇叭用户信息
	*/
    ...
    */
	 ...
	 */
	virtual void RecvNotifyMessage(int msgId,const char* msgContent) = 0;
    //单个礼物图片下载完成通知
	...
	@param size : 列表个数的数量
	*/
	virtual void RecvUserIdxAndType(const ResultData_UIdxWithType* list/*[number]*/,int size) = 0;
	...
	*/
	virtual void RecvUserLoginLobbyNoticeNew(int idx,char type) = 0;
	...
	*/
	virtual void RecvSendHBResult(int ret) = 0;
	...
	*/
	virtual void RecvHongBao(int idx,const char* hbid,char hbtype,const char* remark) = 0;
	 ...
	 */
	virtual void RecvHongBaoStatus(const char* hbid,int ret) = 0;
	/*
	抢红包的结果
	@param result : 抢红包的结果:  0-没抢到  1-抢到 2-已抢 9999-服务器相应超时
	@param idx : 发红包用户的IDX
	@param bhid : 红包ID
	@param hbtype : 红包类型
	@param remark : 红包备注
	...
	*/
	virtual void RecvVieHongBaoResult(int result,int idx,const char* hbid,char hbtype,const char* remark,int gold) = 0;
	...
	@param num : list列表的数量
	*/
	virtual void RecvHongBaoDetail(int ret,int idx,char hbtype,const char* remark,int totalgold,int totalnum
		,int sendtime,const ResultData_HBDetail* list/*[num]*/,int num) = 0;
	...
	@param score : 分数
	*/
	virtual void RecvSingSongChange(const char* songname,const char* singername,int score) = 0;

	/*
	通知红包数量更新
	@param number : 可领取的红包数量
	*/
	virtual void RecvHongBaoNum(int number) = 0;
	...
	@param list : 红包列表
	*/
	virtual void RecvRoomHB(int ret,int number,ResultData_RoomHB* list) = 0;
