...
#ifndef CJNIRESPONSE__H__
#define CJNIRESPONSE__H__
...
typedef std::map<std::string,jclass> MAPSTRCLASS;
typedef std::map<std::string,jfieldID> MAPSTRFIELDID;

class CDataMgr;
class CServerData;
class CJniResponse : public ResponseBase
{
private:
	CJniResponse(void);
	virtual ~CJniResponse(void);

public:
	static CJniResponse* getSingleton();
	//存入class 字典，用于多线程使用
	void initMapClass(JNIEnv* env);
	//初始化Java环境变量，用于线程的回调，
	bool initJniEnv(JNIEnv* env, jobject obj);
	//回收资源
	void uninitJniEnv(JNIEnv* env);
	//
	jobject getRoomInfo(JNIEnv* env,int roomid);

	//在native代码中，如果线程需要使用jnienv环境变量，需要先附加到当前线程，注：jnienv只在本线程使用
	bool attachCurThread(unsigned int threadid);
	//线程退出时务必分离
	bool detachCurThread();
	//获取线程的java环境变量
	bool getJniByThreadID(unsigned int threadid,JNIEnv** penv);
	...
	*/
	...
	通知获取到的喇叭用户信息
	*/
	virtual void RecvUserInfoForSpeaker(int errorcode,ResultData_UserSampleInfo* info);
...
private:
	JavaVM*  m_jvm;  
	//java里面只有一个对象，用于处理java与c的交互
	jobject     m_obj;  

	MAPINTJNIENV m_mapEnv;
	//控制信息
	...
	//类查询字典
	MAPSTRCLASS m_mapClass;

public:
	//登陆的大厅
	int		m_nLobbyId;
	//本地模拟数据库，保存所有信息，只在内存中
	CDataMgr* 	m_pDataMgr;
};

#endif//CJNIRESPONSE__H__
